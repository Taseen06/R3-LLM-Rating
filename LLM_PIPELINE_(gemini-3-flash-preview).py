...
def has_valid_scores(result):
    return result.get("screen_mean") is not None
...
def read_valid_processed_ids(path):
    if not os.path.exists(path):
        return set()
    try:
        df = pd.read_csv(path, dtype={"screen_id": str})
        for col in OUTPUT_COLUMNS:
            if col not in df.columns:
                df[col] = None
        valid = df["screen_mean"].notna()
        return set(df.loc[valid, "screen_id"].dropna())
...
def save_batch_results(path, new_results, selected_order, label):
    ...
    try:
        ...
        # Rebuild from selected_15k.csv order so the judged CSV always follows it exactly.
        selected_ids = pd.DataFrame({"screen_id": list(selected_order.keys())})
        combined = selected_ids.merge(combined, on="screen_id", how="inner")
...
def log_retry(label, sid, attempt, error=None):
    if attempt >= RETRY_LIMIT:
        return
    detail = f": {error}" if error else ""
    print(f"[RETRY {label}] {sid} attempt {attempt + 1}/{RETRY_LIMIT + 1}{detail}", flush=True)
    ...
    if img_path:
        ...
        for attempt in range(RETRY_LIMIT + 1):
            try:
                ...
                result = {"screen_id": sid, **parse_response(response.text or "")}
                if has_valid_scores(result):
                    jpeg_list.append(result)
                    break
                log_retry("JPEG", sid, attempt, "empty or unparseable response")
            except Exception as e:
                if attempt >= RETRY_LIMIT:
                    if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                        print(f"[ERROR JPEG] {sid}: quota exhausted after retries", flush=True)
                    else:
                        print(f"[ERROR JPEG] {sid}: {type(e).__name__}: {str(e)[:100]}", flush=True)
                else:
                    log_retry("JPEG", sid, attempt, f"{type(e).__name__}: {str(e)[:100]}")
            if attempt < RETRY_LIMIT:
        ...
        for attempt in range(RETRY_LIMIT + 1):
            try:
                ...
                if has_valid_scores(result):
                    ...
                    break
                log_retry("JSON", sid, attempt, "empty or unparseable response")
            except Exception as e:
                if attempt >= RETRY_LIMIT:
                    if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                        print(f"[ERROR JSON] {sid}: quota exhausted after retries", flush=True)
                    else:
                        print(f"[ERROR JSON] {sid}: {type(e).__name__}: {str(e)[:100]}", flush=True)
                else:
                    log_retry("JSON", sid, attempt, f"{type(e).__name__}: {str(e)[:100]}")
            if attempt < RETRY_LIMIT:
                time.sleep(RETRY_DELAY_SECONDS)
    else:
...
async def main():
    ...
    jpeg_processed = read_valid_processed_ids(OUTPUT_JPEG_CSV)
    json_processed = read_valid_processed_ids(OUTPUT_JSON_CSV)
